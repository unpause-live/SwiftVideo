...
//
// With the Rtmp system, onConnection is called after an RTMP publish or subscribe handshake occurs.  The workspaceToken
// contains the  "playpath" (or stream key) portion of the URI and the "workspaceId" contains the "app" portion of the
// URI. You must return a future that emits either true or false, depending on if permission is granted to either
// publish or subscribe. Note that the nomenclature "publisher" and "subscriber" are from the perspective of the media
// server, therefore when the peer ispublishing, the server is subscribing.
//
...
let onConnection: LiveOnConnection = { pub, sub in
    if let pub = pub, let streamKey = pub.workspaceToken() {
        print("publisher asking for permission: \(pub.workspaceId())/\(streamKey). Dropping.")
        return Future { $0(.success(false)) }
    }
    if let sub = sub, let src = sub as? Source<CodedMediaSample>, let streamKey = sub.workspaceToken() {
        print("subscriber asking for permission: \(sub.workspaceId())/\(streamKey)")

        // In order to receive samples, we need to compose the subscriber with a receiver and keep a strong reference.
