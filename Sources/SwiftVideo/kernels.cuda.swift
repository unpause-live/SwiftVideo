...
__device__ void write_imagef(unsigned char* buf, const int2 gid, const float value) {
    const int offset = gid.y * gridDim.x + gid.x;
...
enum CUDAKernel: String, CaseIterable {
  case img_clear_y420p =
    ...
    PRELUDE void img_clear_y420p(unsigned char* outLuma,
      ...
      write_imagef(outLuma, gid, 0.f);
      write_imagef(outChromaU, div(gid,2), 0.5f);
      write_imagef(outChromaV, div(gid,2), 0.5f);
    }
    """
  case img_bgra_y420p =
    """
    PRELUDE void img_bgra_y420p(unsigned char*  outLuma,
        ...
        if(border.x >= 0.f && border.y >= 0.f && border.x <= 1.f &&  border.y <= 1.f) {
            ...
            float4 curV;
            if(handleChroma) {
                curU = read_imagef(curChromaU, curSampler, div(gid, 2));
                curV = read_imagef(curChromaV, curSampler, div(gid, 2));
            }
            if(tx.x >= 0.0 && tx.y >= 0.0 && tx.x <= 1.0 && tx.y <= 1.0) {
                ...
                if(uv.x >= 0.f && uv.y >= 0.f && uv.x <= 1.f && uv.y <= 1.f) {
                    ...
                    float4 bgra = read_image4f_bilinear(inPixels, 
  ...
  case img_y420p_y420p =
    ...
    PRELUDE void img_y420p_y420p(unsigned char* outLuma,
        ...
        if(border.x >= 0.f && border.y >= 0.f && border.x <= 1.f &&  border.y <= 1.f) {
            ...
            if(tx.x >= 0.0 && tx.y >= 0.0 && tx.x <= 1.0 && tx.y <= 1.0) {
                if(uv.x >= 0.f && uv.y >= 0.f && uv.x <= 1.f && uv.y <= 1.f) {
                    float4 luma = read_imagef(inLuma, sampler, make_float2(uv.x, uv.y));
                    float alpha = uniforms->opacity;
                    write_imagef(outLuma, gid, (curY * (1.f - alpha) + luma * alpha));
                    if(handleChroma) {
                        float4 cb = read_imagef(inChromaU, sampler, make_float2(uv.x, uv.y));
                        float4 cr = read_imagef(inChromaV, sampler, make_float2(uv.x, uv.y));
                        write_imagef(outChromaU, div(gid, 2), (curU * (1.f - alpha) + cb * alpha));
                        write_imagef(outChromaV, div(gid, 2), (curV * (1.f - alpha) + cr * alpha));
                    }
                    return;
                }
            }
            ...
            if(handleChroma) {
                write_imagef(outChromaU, div(gid, 2), clamp((curU * (1.f - alpha) + fillColor.y * alpha), -1.f, 1.f));
                write_imagef(outChromaV, div(gid, 2), clamp((curV * (1.f - alpha) + fillColor.z * alpha), -1.f, 1.f));
            }
        }
    }
    """
